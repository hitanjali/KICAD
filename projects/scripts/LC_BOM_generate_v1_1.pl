...
print `clear`;
print "Enter the path of the folder that containts the xml file : \n";
chomp($proj_path = <STDIN>);

chdir($proj_path);

chomp($no_of_xmls = `ls  *.xml | wc -l`);

if($no_of_xmls > 1) {

	print "================================================================================\n";
	print "! ! ! ! ! ! ! ! ! ! ! More than one XML present ! ! ! ! ! ! ! ! ! ! !  \n\n";
	print `ls  *.xml`;
	print "\n\n";
	print "Enter the name of the XML file to be processed : ";
	chomp($xml_file_name = <STDIN>);
}
elsif ($no_of_xmls == 1) {
	chomp($xml_file_name = `ls *.xml`);
	print "\nProcessing xml file : $xml_file_name\n";
}
else {
	die "! ! ! ! ! ! ! ! ! ! ! No XML files present in the given path ! ! ! ! ! ! ! ! ! ! !  ";
}
	


@arr = split /\./ , $xml_file_name;
